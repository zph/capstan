...
class Node {
  ...
  constructor({name, state, port, node_type, DESIRED_VERSION}: {name: string, state: NodeState, port: number, node_type: NodeType, DESIRED_VERSION: string}) {
    this.name = name
    this.state = state
    this.port = port
  ...
  async role() {
    return JSON.parse(await this.#mongo("rs.status()")).members.find((v) => v.self).stateStr
  }
  ...
  async getFeatureControlVersion() {
    const response = JSON.parse(await this.#mongo(`db.adminCommand({getParameter: 1, featureCompatibilityVersion: 1})`))
  ...
  async upgradeFeatureControlVersion(mongod: Node) {
    ...
    const r = await confirm(`Are you sure you want to set fcv: ${fcv} from ${currentVersion}`)
    if(!r) {
      console.log(`Refused upgrade, skipping`)
      return
    }
    await this.#mongo(`db.adminCommand({ setFeatureCompatibilityVersion: '${fcv}' })`)
  }

  // TODO: rework this to use plain binary and the --binarypath flag
  async stop() {
    await $.raw`./bin/mlaunch stop ${this.node_type} ${this.port}`.env({MONGO_VERSION: this.DESIRED_VERSION})
  }

  async start() {
    await $.raw`./bin/mlaunch start ${this.node_type} ${this.port}`.env({MONGO_VERSION: this.DESIRED_VERSION})
  }
  ...
  async updateStatus() {
    ...
    const mongod = processes.find(p => p.command.includes(this.node_type) && p.command.includes(`${this.port}`))
    try {
      if (mongod) {
        this.state = "running"
      } else {
  ...
  async #mongo(cmd: string) {
    return (await $.raw`mongo --quiet mongodb://localhost:${this.port} --eval "print(JSON.stringify(${cmd}))"`.text()).split("\n").filter((v) => !v.match("machdep.cpu"))?.join("\n")
  }
}
...
class Processes {
  async load() {
    ...
    return raw.split("\n").slice(1).map(this.processLine).filter(p => p.command)
  }
...
while (true) {
  const buildNotVersionCheck = (n: Node) => new Check({
  ...
  const isSecondaryCheck = (n: Node) => new Check({
  ...
  const isPrimaryCheck = (n: Node) => new Check({
  ...
  const isOnlineCheck = (n: Node) => new Check({
  ...
  const buildIsVersionCheck = (n: Node) => new Check({
  ...
  const buildInHealthyReplicaCheck = (n: Node) => new Check({
    name: `[${n.name}] mongod in a healthy replicaset`,
    fn: async () => {
      const states = (await n.rs_status()).members.map(m => m.stateStr)
      return (
        states.every(s => ["PRIMARY", "SECONDARY"].includes(s)) && [1, 3, 5].includes(states.length)
      )
    },
  })

  const upgradeMongoCommand = (n: Node) => new Command({
      name: "change mongo version",
      fn: async () => {
        await $`./bin/mlaunch stop ${n.node_type} ${n.port}`.env({MONGO_VERSION: n.DESIRED_VERSION})
        await new Promise(resolve => setTimeout(resolve, 2000))
        await $`./bin/mlaunch start ${n.node_type} ${n.port}`.env({MONGO_VERSION: n.DESIRED_VERSION})
        await new Promise(resolve => setTimeout(resolve, 5000))
      }})


  const buildUpgradeMongodAction = (n: Node, configServers: Node[]) => new Action({
  ...
  const buildUpgradeConfigServerAction = (n: Node) => new Action({
    name: `[${n.name}] upgrade mongo to ${n.DESIRED_VERSION}`,
    preChecks: [
      isOnlineCheck(n),
      isSecondaryCheck(n),
  ...
  const buildUpgradeMongosAction = (n: Node, nodes: Node[]) => new Action({
  ...
  const isMongoOnline = (n: Node) => new Check({
  ...
  const startServersAction = (n: Node) => new Action({
  ...
  const nodes = [...Array(9).keys()].map(i => i + 27020).map((port) => new Node({
    "name": `mongod:${port}`,
    "node_type": "mongod",
    "state": "unknown",
    port,
  ...
  const configServers = [...Array(3).keys()].map(i => i + 27029).map((port) => new Node({
  ...
  const mongoses = [...Array(3).keys()].map(i => i + 27017).map((port) => new Node({
    "name": `mongos:${port}`,
    "node_type": "mongos",
    "state": "unknown",
    port,
  ...
  const buildFailoverToUpgrade = (n: Node) => new Action({
  ...
  const UpgradeFeatureControlVersionAction = (mongod: Node, mongos: Node[]) => new Action({
  ...
  const configServerUpgrades = configServers.map(buildUpgradeConfigServerAction)
  const mongodUpgrades = nodes.map(n => buildUpgradeMongodAction(n, configServers))
  const mongosUpgrades = mongoses.map(n => buildUpgradeMongosAction(n, [...configServers, ...nodes]))
  const startServers = [...nodes, ...configServers, ...mongoses].map(startServersAction)

  const actions = [...startServers,
